<%@ WebHandler Language="C#" Class="card_view" %>

using System;
using System.Web;
using System.Data;

public class card_view : IHttpHandler {
    dal dal = new dal();
    common common = new common();
    public void ProcessRequest (HttpContext context) {
        string guid = (context.Request["guid"] != null) ? context.Request["guid"].ToString() : "";
        DataTable dt = dal.card_veiw(guid);
...

        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                xml_sql += "<cd_user>" +common.GetEmpName(dt.Rows[i]["cd_user"].ToString()) + "</cd_user>";
                xml_sql += "<cd_host>" + common.GetEmpName(dt.Rows[i]["cd_host"].ToString()) + "</cd_host>";
                xml_sql += "<cd_host_sle>" + dt.Rows[i]["cd_host"].ToString() + "</cd_host_sle>";
