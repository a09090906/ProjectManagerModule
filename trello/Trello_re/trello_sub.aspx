...
<div id="editcardblock" class="magpopup magSizeM mfp-hide">
  ...
  <div class="padding10ALL">
      ...
      <div class="col-lg-6 col-md-6">
	    ...
	    <div id ="sub_list_div" guid="" style="display:none">         
            目前進度:
             <select id="sub_list"></select>
	    </div>
        <p id="sa_p"></p>
        卡片負責人:
...
<script type="text/javascript">   
    ...
    $(document).ready(function () {
        if (ex_schedule() == 'ex3') {
            ...
            //        url: "ashx/sub_list_chenge.ashx",
            //        data: {
            //            s_guid: $("#sub_list").val(),
            //            cd_guid: $("#save").attr("guid")
            //        },
            //        error: function (xhr) {
                ...
                success: function (data) {
                    ...
                    if ($(data).find("data_item").length > 0) {
                        $(data).find("data_item").each(function (i) {
                            $("#sub_list").val($(this).children("cd_sub").text().trim())
                ...
                if (data != null) {
                    ...
                    if ($(data).find("data_item").length > 0) {                       
                        $(data).find("data_item").each(function (i) {
                            ...
                            tabstr += "<ul  class='sonsortable' guid=" + $(this).children("s_guid").text().trim() + " ><li style='height:10px;'></li></ul> <div class='margin5T font-normal'guid=" + $(this).children("s_guid").text().trim() + " ><a href='#' class='addcardbtn'>新增卡片</a></div></div ></li >";
                        });
    ...
    //判斷是否為範本進度追蹤
    function ex_schedule() {
        var ex;
        ex = '';
        $.ajax({
            type: "POST",
            async: false, 
            url: "ashx/sub_ex.ashx",
            data: {
                guid: getParameterName('id')
            },
            error: function (xhr) {
                alert("Error " + xhr.status);
                console.log(xhr.responseText);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data != null) {
                    if ($(data).find("data_item").length > 0) {
                        $(data).find("data_item").each(function (i) {
                            tabstr += "<option value =" + $(this).children("s_guid").text().trim() + ">" + $(this).children("s_name").text().trim() + "</option>";
                        });
                    }
                }
                $("#sub_list").append(tabstr)
            }
        });
    ...
    function card_veiw(cd_guid) {    
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if ($(data).find("data_item").length > 0) {
                    $(data).find("data_item").each(function (i) {
                        //$("#sub_list option[value=" + $(this).children("cd_sub").text().trim() + "]").attr('selected', 'selected');
                        $("#sub_list").val($(this).children("cd_sub").text().trim());
                        $("#save").attr("guid", $(this).children("cd_guid").text().trim());
                        $("#card_title").val($(this).children("cd_title").text().trim());
                        $("#sa_p").text('建立者:'+$(this).children("cd_user").text().trim());
